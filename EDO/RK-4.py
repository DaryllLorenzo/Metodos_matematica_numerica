...
def rk4(x0,y0,xf,h):
    ...
    K2 = h * eval_funcion_problema(x0 + 1/2 * h , y0 + 1/2 * K1)
    K3 = h * eval_funcion_problema(x0 + 1/2 * h , y0 + 1/2 * K2)
    K4 = h * eval_funcion_problema(x0 + h,y0 + K3)

    Ysgt = y0 + 1/6 * (K1 + 2*K2 + 2*K3 + K4)
    Xsgt = x0 + h

    print(f"n: {n} / Xn: {x0} / Yn: {y0} / f(Xn, Yn): {eval_funcion_problema(x0, y0)} / Xn+1: {x0 + h} / Yn+1: {Ysgt} / K1: {K1} / K2: {K2} / K3: {K3} / K4: {K4}")
    n = n + 1
    #x0 = Xsgt
    #y0 = Ysgt
...
def error_doble_calculo(x0,y0,xf,h):
    print("")
    error = ( rk4(x0,y0,xf,h) - rk4(x0,y0,xf,h * 2) ) / 15
...
def error_doble_calculo_intervalo(x0,y0,xf,h):
    ...
    while n != tam:
        if n % 2 == 0:
           print(f"Error de n = {n} es: {(yh[n] - y2h[i]) / 15}")
           dic[(n,i)] = (yh[n] - y2h[i]) / 15
           arr_err.append((yh[n] - y2h[i]) / 15)
           i += 1
